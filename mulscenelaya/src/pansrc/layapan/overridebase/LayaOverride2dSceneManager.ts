...
module topfront {
    ...
    export class LayaForntPanel extends UIConatiner {
        ...
        public drawDemo(v2d: Pan3d.Vector2D): void {
            //this.addA(v2d)
            //this.addB(v2d)
            //this.addC(v2d)
        ...
        public drawDynamicTextDemo(): DynamicTextMeshVo {
            var vo: DynamicTextMeshVo = new DynamicTextMeshVo;
            vo.label = "[ff0000]" + "中华人民共和国"  //必须
            vo.fontsize = 20;  //文字大小  (可选)
            vo.tx = 0;  //位置偏移(可选)
            vo.ty = 30;  //位置偏移(可选)
            vo.bg = "ui/textlist/128_64.png"  //背景图(可选)
            vo.bgRect = new Pan3d.Rectangle(0, 0, 256, 64)  //暂时不能超过256*64(可选)
            vo.showTime = 2000;
            vo.alpha=0.2
        ...
        public constructor() {
            ...
            this._baseRender.uiAtlas.loadImgUrl("ui/textlist/textlistabc.png", () => { this.loadConfigCom() })
        }
        ...
        private loadConfigCom(): void {
            ...
            this.addNum0_9("greedA", new Rectangle(0, 24, 20, 24));
            this.addNum0_9("greedB", new Rectangle(0, 48, 20, 24));
            ...
            this.makeFrontUiRect("wenzibeijing", new Rectangle(0, 295, 130, 31));
        ...
        private addB(v2d: Pan3d.Vector2D): void { //组合，有底，有前部分和数字，注意层级
            ...
            c.x = b.x + 10
            c.y = b.y + 30
            var d: BaseFrontVo = this.drawLabel(1, { color: "yellow", num: 8888 }, false);
            d.x = b.x + 100
            d.y = b.y + 30
            d.alpha = 1;
        }
        ...
        public numStrVo(color: string, num: number, isbottom: boolean): BaseFrontVo {
            var vo: ColorFrontVo = new ColorFrontVo(this, isbottom);
            vo.makeUi({ color: color, num: num })
            return vo
        }
        //第一个参数为类开，1为数字，2为显示单图  //第二个为参数，第二个显上下层位置
        private drawLabel(type: number, data: any, isbottom: boolean = false): BaseFrontVo {
